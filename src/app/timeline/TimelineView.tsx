...
export default function TimelineView({ objects, civilizations }: Props) {
  ...
  const logDensity = density.map(d => Math.log1p(d));
  const maxLog = Math.max(...logDensity, 1);
            ...
            <div className="absolute inset-0 flex items-end gap-px px-0">
              {logDensity.map((ld, i) => (
                <div
                  ...
                  style={{
                    height: `${(ld / maxLog) * 100}%`,
                    backgroundColor: `rgba(100,90,80,${0.15 + (ld / maxLog) * 0.55})`,
                  }}
                />
              ))}
            </div>
        ...
        {civilizations.map((civ) => {
          ...
          return (
            <div key={civ.id}>
              ...
              {/* Objects row */}
              <div className="grid gap-2 sm:gap-3 grid-cols-3 sm:grid-cols-4 md:grid-cols-5 lg:grid-cols-7">
                {civObjects.map((obj) => (
                  <Link
                    key={obj.id}
                    href={`/object/${obj.id}`}
